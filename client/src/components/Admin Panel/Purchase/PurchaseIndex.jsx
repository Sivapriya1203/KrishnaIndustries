...
import AddPurchase from "./AddPurchase";
import UpdatePurchase from "./UpdatePurchase";
import UploadExcell from "./UploadExcell"; // Import the UploadExcell component
import axios from "axios";
import Search from "../../Search Option/Search";
import Stack from '@mui/material/Stack';
import Pagination from '@mui/material/Pagination'
...
const PurchaseIndex = () => {
  ...
  const [openInvoice, setOpenInvoice] = useState(false);
  const [invoiceData, setInvoiceData] = useState([]);
  const [openUpload, setOpenUpload] = useState(false); // Add state for UploadExcell
  ...
  return (
    <div>
      ...
      <Grid container spacing={3} className="mt-3">
        ...
        <Grid item xs={3} display="flex" justifyContent="center">
          <Button onClick={() => setOpenUpload(true)}>Upload Excel</Button> {/* Add button for UploadExcell */}
        </Grid>
        <Grid item xs={3} display="flex" justifyContent="center">
          <Search data={purchData} setData={setSearchPurchData} />
        </Grid>
      ...
      <Dialog open={openUpload} onClose={() => setOpenUpload(false)} maxWidth="md"> {/* Add Dialog for UploadExcell */}
        <DialogContent>
          <UploadExcell onClose={() => setOpenUpload(false)} />
        </DialogContent>
        <DialogActions>
